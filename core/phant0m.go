...
func phant0m(eventlog_pid uint32, verbose bool) (error) {
  ...
  if hThreads == 0 {
    return errors.New("An error has ocurred calling CreateToolhelp32Snapshot")
  }
  ...
  for true {
    if (te32.OwnerProcessID == eventlog_pid) {
      ...
      if hEvtThread == 0 {
        return errors.New("An error has ocurred calling OpenThread")
      }
      ...
      if hEvtProcess == 0 {
        fmt.Println("2")
        return errors.New("An error has ocurred calling OpenProcess")
      }
